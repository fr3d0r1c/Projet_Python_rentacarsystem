...
class Horse(TransportAnimal):
    ...
    @property
    def category(self):
        return "Poney" if self.wither_height < 140 else "Cheval"

    def show_details(self):
        return f"[{self.category}] {self.name} ({self.age} ans) - {self.wither_height}cm"

    def to_dict(self):
        data = super().to_dict()
        data.update({"age": self.age, "wither_height": self.wither_height, "shoe_size_front": self.shoe_size_front, "shoe_size_rear": self.shoe_size_rear})
        return data
...
class Donkey(TransportAnimal):
    ...
    def show_details(self):
        c = "Têtu" if self.is_stubborn else "Docile"
        return f"[Âne] {self.name} ({self.age} ans) - {c}"

    def to_dict(self):
        data = super().to_dict()
        data.update({"age": self.age, "pack_capacity_kg": self.pack_capacity_kg, "is_stubborn": self.is_stubborn})
        return data
...
class Camel(TransportAnimal):
    ...
    def to_dict(self):
        data = super().to_dict()
        data.update({"age": self.age, "hump_count": self.hump_count, "water_reserve": self.water_reserve})
        return data
...
class Whale(TransportAnimal):
    def __init__(self, t_id, daily_rate, name, breed, age, weight_tonnes, can_sing):
        super().__init__(t_id, daily_rate, name, breed, None)
        self.age = age
        self.weight_tonnes = weight_tonnes
        self.can_sing = can_sing

    def show_details(self):
        s = "Chante" if self.can_sing else "Muette"
        return f"[Baleine] {self.name} ({self.age} ans) - {self.weight_tonnes}T - {s}"

    def to_dict(self):
        data = super().to_dict()
        data.update({"age": self.age, "weight_tonnes": self.weight_tonnes, "can_sing": self.can_sing})
        return data
...
class Dolphin(TransportAnimal):
    def __init__(self, t_id, daily_rate, name, breed, age, swim_speed, knows_tricks):
        ...
        self.knows_tricks = knows_tricks

    def show_details(self):
        t = "Savant" if self.knows_tricks else "Sauvage"
        return f"[Dauphin] {self.name} - {self.swim_speed}km/h - {t}"

    def to_dict(self):
        data = super().to_dict()
        data.update({"age": self.age, "swim_speed": self.swim_speed, "knows_tricks": self.knows_tricks})
        return data
...
class Eagle(TransportAnimal):
    ...
    def show_details(self):
        return f"[Aigle] {self.name} - Env: {self.wingspan_cm}cm"

    def to_dict(self):
        data = super().to_dict()
        data.update({"age": self.age, "wingspan_cm": self.wingspan_cm, "max_altitude": self.max_altitude})
        return data
...
class Dragon(TransportAnimal):
    ...
    def to_dict(self):
        data = super().to_dict()
        data.update({"age": self.age, "fire_range": self.fire_range, "scale_color": self.scale_color})
        return data